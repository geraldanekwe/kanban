...
const TaskModalComponent: React.FC<TaskModalProps> = (props) => {
  ...
  const allTags = useMemo(
    () => (mode === "add" || mode === "edit" ? props.allTags : []),
    [mode, props]
  );

  const allAssignees = useMemo(
    () => (mode === "add" || mode === "edit" ? props.allAssignees : []),
    [mode, props]
  );
  useEffect(() => {
    if (mode === "edit") {
  ...
  const addTag = useCallback(
    (tag: string) => {
      if (!tag.trim() || mode === "delete") return;
  ...
  const handleSubmit = useCallback(
    (e: React.FormEvent) => {
      ...
      const task: Task = {
        id: mode === "edit" ? props.selectedTask.id : Date.now().toString(),
        title,
        description,
        status,
        assignee,
    ...
    [title, description, assignee, tags, status, mode, props, onClose]
  );
  ...
  }, [title, description, assignee, tags, status, mode, props]);

  if (!isOpen) return null;

  if (mode === "delete") {
    const { onDeleteTask, selectedTask } = props;
    return (
      <div
        ...
        <div className="p-6">
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <div className="flex justify-end gap-3 pt-4">
              ...
              <button
                type="submit"
                disabled={mode === "edit" && isUnchanged}
